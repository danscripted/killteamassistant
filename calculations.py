...
def operative_menu():
    ...
    # FIXED
    weapons = operatives[chosen]["weapons"]
    for index, name in enumerate(weapons, start=1):
        print(f"{index}. {name}")
    weapons_list = list(weapons.keys())
    selection_weapon = input("Select a weapon: ")
    chosen_weapon = weapons_list[int(selection_weapon) - 1]
    display_operative(operatives[chosen]) 
